                  ...
                  <div class="panel panel-default">
                    <div class="panel-body">
                      <table class="table table-striped" id="tab_ctt_cmp">
                        ...
                        <?php
                          $sql = $wpdb->prepare($selecionar_contatos_cliente , $id_cmp, $id_cli );
                          $contatos = $wpdb->get_results( $sql );
                            // "
                            // SELECT 
                            // (select count(cd_vcl_ctt_cmp) from `VCL_CTT_CMP` 
                            //where `VCL_CTT_CMP`.cd_ctt=CONTATO.cd_ctt and `VCL_CTT_CMP`.cd_cmp={$id_cmp} and `VCL_CTT_CMP`.ativo=1) as total,
